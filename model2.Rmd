...
noshow2$ddif <- as.numeric(noshow2$ddif)
#noshow$ddif <- as.numeric(noshow$ddif)

#hist(noshow2$ddif)

noshow2$Age <- cut_number(noshow2$Age,6, labels=c("4-37","38-45","46-50","51-55","56-63",">63"))
noshow2$ddif <- cut(noshow2$ddif,c(-Inf,0,7,Inf), labels = c("Same day", "Same week", ">7 days"))
summary(noshow2)
...
noshow2 %>% 
  ggplot(aes(x=No.show, fill=ddif))+
  geom_bar(position = "fill") +  
  theme(text = element_text(size=20)) +
  xlab("No Show") +
  ylab("proportion") +
...
#baseline model
bl <- table(noshow2$No.show)
bl
bl[1]/(bl[1]+bl[2])*100

#splitting the data
set.seed(88)
split <- sample.split(noshow2$No.show, SplitRatio = 0.80)

#create training and test data
noshow_train <- subset(noshow2, split == T)
noshow_test <- subset(noshow2, split == F)
...
spec_table <- table(noshow_train$No.show, predict_train >=0.5)
spec_table
...
test_tble <- table(noshow_test$No.show,predict_test >=0.3)
test_tble
...
write.csv(df,"cal2.csv")
cal2 <- read.csv("cal2.csv")

str(cal2)


calibration_plot(cal2$B, cal2$A)
